...
The Worker accepts `POST /v1/import-html` with `{ "shareUrl": string }` from its configured browser Origin. It rate counts requests before reading the JSON body, caps the body at 4,096 bytes including unknown-length streams, then validates the initial URL against three enabled versioned canonical templates and calls `fetch` once with GET, `redirect: "follow"`, omitted credentials, no caller headers, and no cache. The ChatGPT template remains in the registry but is disabled and rejected before fetch. The runtime may follow multiple redirects to any destination. The Worker does not inspect or restrict redirect hops; runtime failures are mapped to an atomic typed error.
...
- Enabled initial templates: `claude.ai/share/<uuid>`, `chat.mistral.ai/chat/<uuid>`, and `share.gemini.google/<12-alphanumeric-id>`. The retained `chatgpt.com/share/<uuid>` template has `enabled: false` under policy version `2026-09-23.2`. Initial URL limit: 2,048 characters.
...
- Controlled tests cover the three enabled templates, disabled ChatGPT input, automatic redirect behavior, invalid input, CORS, isolation, limits, and atomic errors. An optional deployed smoke check accepts operator-supplied endpoint, Origin, and share URL.
